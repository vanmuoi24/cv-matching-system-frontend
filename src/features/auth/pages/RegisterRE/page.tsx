...
const Register = () => {
	...
	const handleRegister = async () => {
		...
		setLoading(true);
		try {
			const response = await RegisterApi({ fullName, email, password, confirmPassword, role: 'RECRUITER' });
			if (response && response.result) {
				message.success('Đăng ký nhà tuyển dụng thành công! Vui lòng đăng nhập.');
				navigate('/auth/re/login');
			}
		} catch (error: any) {
			console.error(error);
				...
				<div className='flex-1'>
					<div className='py-6 px-10'>
						...
						<div className='mb-6 space-y-4'>
							<div>
								<label className="block text-sm font-medium text-gray-700 mb-1">Tên công ty / Người đại diện</label>
								<input
									type='text'
									...
									placeholder='Nhập tên công ty hoặc người đại diện'
									value={fullName}
						...
						<div className='text-center mb-4'>
							<p className='text-sm'>
								Đã có tài khoản? <Link to='/auth/re/login' className='text-[#5932bc] font-semibold hover:underline'>Đăng nhập ngay</Link>
							</p>
						</div>
